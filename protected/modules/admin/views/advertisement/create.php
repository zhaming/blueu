...
<div class="row">
    <div class="col-xs-12">
        ...
        <form action="/admin/advertisement/create" method="POST" enctype="multipart/form-data" class="form-horizontal">
            ...
            <div class="space-4"></div>
            <div class="form-group">
                <label class="col-sm-3 control-label no-padding-right" for="form-field-source"><?php echo Yii::t('admin', 'Source'); ?></label>
                <div class="col-sm-9">
                    <select id="form-field-source" name="advertisement[source]" class="col-sm-5 no-padding-left">
                        <?php foreach (HelpTemplate::getAdSource() as $key => $value) { ?>
                        <option value="<?php echo $key; ?>"><?php echo $value; ?></option>
                        <?php } ?>
                    </select>
                </div>
